 ...
 *
 * - Persists preference to `localStorage` under the key `applirank-color-mode`.
...
export function useColorMode() {
  ...
  function toggle() {
    ...
    if (import.meta.client) {
      localStorage.setItem('applirank-color-mode', colorMode.value)
    }
  }
  ...
  function set(mode: 'light' | 'dark') {
    ...
    applyClass()
    if (import.meta.client) {
      localStorage.setItem('applirank-color-mode', mode)
    }
  }
  ...
  if (import.meta.client) {
    onMounted(() => {
      const stored = localStorage.getItem('applirank-color-mode') as 'light' | 'dark' | null
      if (stored) {
